...
CDBLTDD.prototype.getUser = async function(user){
  const u = await this.db_.collection("database_sv").findOne({username:user});
  return u;   // trả về user hoặc null
}
// hàm lấy danh sách tất cả 
CDBLTDD.prototype.getUsers = async function(){//không trả về _id
  const users = await this.db_.collection("database_sv").find({}, { projection: { _id: 0 } }).toArray();
  return users;   // trả về mảng users
}
// hàm xác thực đăng nhập
CDBLTDD.prototype.Authentication = async function(user, pass){
  const u = await this.db_.collection("database_sv").findOne({ username: user, password: pass});
  return u;   // trả về user hoặc null
}
// hàm cập nhật thông tin người dùng
CDBLTDD.prototype.modifyUser = async function (user, modify){
    //console.log(modify);
    const oDoc = await this.db_.collection("database_sv").updateOne({username:user},{$set:modify});
    //console.log(oDoc);
    return oDoc;
} 
...
CDBLTDD.prototype.Register = async function (user, pass, email, fullname) {
    try {
        ...
        const existingUser = await this.db_.collection("database_sv").findOne({ username: user });
        if (existingUser) {
        ...
        const result = await this.db_.collection("database_sv").insertOne({
            username: user,
            password: pass,
            email: email,
            fullname: fullname,  
