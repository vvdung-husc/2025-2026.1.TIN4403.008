...
CDBLTDD.prototype.registerUser = async function (userInfo) {
    try {
        // Chèn một user mới vào collection "users"
        const result = await this.db_.collection("users").insertOne(userInfo);
        return result; // Trả về kết quả (bao gồm insertedId)
    } catch (error) {
