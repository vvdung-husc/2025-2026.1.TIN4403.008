...
var appRouter = function (app) {
  ...
  app.post("/login", async function (req, res) {
    ...
    await POST_login(user, pass, res);
  ...
  await POST_register(user, pass, name, email, res);
});
...
async function POST_register(username, password, fullname, email, res) {

  if (!username || username.length < 3) {
    UTILS.apiResult(-1, "Tên tài khoản phải >= 3 ký tự", res);
    return;
  }
  ...
  if (!result.inserted) {
    UTILS.apiResult(-3, result.message || "Không thể đăng ký", res);
    return;
  }
