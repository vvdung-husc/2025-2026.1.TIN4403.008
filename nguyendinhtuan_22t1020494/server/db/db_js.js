...
CDBLTDD.prototype.getUser = async function(user){
  const u = await this.db_.collection("students").findOne({username:user});
  return u;   // trả về user hoặc null
}
 
CDBLTDD.prototype.getUsers = async function(){ // lấy tất cả danh sách, không trả về _id
  const users = await this.db_.collection("students").find({}, { projection: { _id: 0 } }).toArray();
  return users;   // trả về mảng users
}

CDBLTDD.prototype.Authentication = async function(user, pass){
  const u = await this.db_.collection("students").findOne({ username: user, password: pass});
  return u;   // trả về user hoặc null
}

CDBLTDD.prototype.modifyUser = async function (user, modify){
    //console.log(modify);
    const oDoc = await this.db_.collection("students").updateOne({username:user},{$set:modify}); // cập nhật thông tin người dùng
    //console.log(oDoc);
    return oDoc;
} 
...
CDBLTDD.prototype.Register = async function (user, pass, email, fullname) {
    try {
        ...
        const existingUser = await this.db_.collection("students").findOne({ username: user });
        if (existingUser) {
        ...
        const result = await this.db_.collection("students").insertOne({
            username: user,
            password: pass,
            email: email,
            fullname: fullname,  
