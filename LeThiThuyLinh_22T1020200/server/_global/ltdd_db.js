...
CDBLTDD.prototype.addUser = async function (user, pass, name, email) {
    try {
        // Kiểm tra xem tài khoản đã tồn tại chưa
        const existingUser = await this.db_.collection("users").findOne({ username: user });
        if (existingUser) {
        ...
        return result.acknowledged; // Trả về true nếu chèn thành công
    } catch (error) {
